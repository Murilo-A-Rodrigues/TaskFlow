...
class RemindersLocalDaoSharedPrefs {
  ...
  Future<List<ReminderDto>> listAll() async {
    try {
      ...
      final List<ReminderDto> reminders = jsonList
          .map((jsonItem) => ReminderDto.fromMap(jsonItem as Map<String, dynamic>))
          .toList();
  ...
  Future<void> upsertAll(List<ReminderDto> reminders) async {
    try {
      ...
      print('RemindersLocalDao.upsertAll: ${reminders.length} reminders upserted, total: ${updatedList.length}');
    } catch (e) {
  ...
  Future<void> delete(String reminderId) async {
    try {
      final existing = await listAll();
      final updated = existing.where((reminder) => reminder.id != reminderId).toList();
      await _saveAll(updated);
      
      print('RemindersLocalDao.delete: removed reminder $reminderId, remaining: ${updated.length}');
    } catch (e) {
