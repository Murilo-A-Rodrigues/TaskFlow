...
/// Interface abstrata para persistência local de ProjectDto
/// 
/// Define o contrato para operações de cache local de projetos.
/// Implementações concretas devem fornecer mecanismos de armazenamento
/// (ex: SharedPreferences, SQLite, Hive, etc.).
abstract class ProjectLocalDto {
  ...
  /// 
  /// Parâmetros:
  /// - [dtos]: Lista de ProjectDto para persistir
  Future<void> upsertAll(List<ProjectDto> dtos);
  ...
  /// 
  /// Retorno:
  /// - Lista de ProjectDto do cache local
  Future<List<ProjectDto>> listAll();
  ...
  /// 
  /// Retorno:
  /// - ProjectDto se encontrado, null caso contrário
  Future<ProjectDto?> getById(String id);

  /// Limpa o cache (útil para reset/diagnóstico).
  /// 
  /// Remove todos os registros de projetos do armazenamento local.
