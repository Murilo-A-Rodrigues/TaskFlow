    ...
    try {
      ...
      if (raw != null && raw.isNotEmpty) {
        try {
          ...
          print(
            '⚠️ Dados corrompidos no cache de comentários, reiniciando: $e',
          );
        }
      }
      ...
      print(
        '✅ Cache de comentários atualizado: ${dtos.length} registro(s), total: ${merged.length}',
      );
    } catch (e) {
  ...
  Future<List<CommentDto>> listAll() async {
    try {
      ...
      final comments = jsonList
          .map(
            (json) =>
                CommentDto.fromMap(Map<String, dynamic>.from(json as Map)),
          )
          .toList();

      print(
        '📋 Cache de comentários carregado: ${comments.length} registro(s)',
      );
      return comments;
    } catch (e) {
